...
import { getMatchMutationStatus, joinMatch, MatchMutationError } from "@/lib/match-service";
...
export async function POST(
  ...
  try {
    const access = await joinMatch(user.userId, matchId);

    if (access.status !== "ok") {
      return apiError("Could not load match after joining.", 500);
    }
